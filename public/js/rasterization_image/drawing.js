...
var viewMatrix;             // Viewing transformation (from world to viewing coordinates)
var projectionMatrix;       // Projection transformation (from viewing to normalized coordinates)
...
function draw(){
    ...
    let [camera_position, camera_fov] = get_camera_input();

    // Compute the view and projection matrices based on the camera position and field of view.
    defineViewProjectionMatrices(camera_position, camera_fov);
    ...
    let [light_direction, plight_position, plight_position_vec4, plight_intensity] = get_light_input();
    ...
    set_general_uniforms(light_direction, plight_position_vec4, plight_intensity);
...
function get_light_input(){
    ...
    vec4.transformMat4(light_direction, light_direction, viewMatrix);        // Transform the light direction from wold to view coordinates
    ...
    vec4.transformMat4(plight_position_vec4, plight_position_vec4, viewMatrix);
 ...
 */
function defineViewProjectionMatrices(camera_position, camera_fov){
    ...
    mat4.perspective(projectionMatrix, camera_fov, gl.viewportWidth / gl.viewportHeight, 0.1, 30.0);
}
 ...
 */
function set_general_uniforms(light_direction, plight_position_vec4, plight_intensity){
    ...
    let plightPositionLocation = gl.getUniformLocation(shaderProgram, "pointLightPosition");
    ...
    let plightIntensityLocation = gl.getUniformLocation(shaderProgram, "pointLightIntensity");
 ...
 */
function draw_shapes(plight_position){
